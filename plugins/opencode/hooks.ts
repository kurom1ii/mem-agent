...
export const MEMAGENT_INSTRUCTIONS = [
  ...
  "  Args: query (string), limit (int, mặc định 10),",
  "        mode (fts5|vector|hybrid), scope (auto|chat|tool|all)",
  "  → Dùng khi: user hỏi \"nhớ gì về...\", cần context trước sửa file.",
  "  → Mặc định mode=\"hybrid\", scope=\"auto\".",
  "",
  "mem-agent_memory_add — Lưu memory mới.",
  "  Args: title, content, tags, kind, type, session_id, source, project",
  "  → Dùng khi: fix bug xong, chốt quyết định, học convention mới.",
  "",
  "mem-agent_memory_list — Danh sách memory gần đây.",
  "  Args: limit (int, mặc định 20), scope (all|chat|tool)",
  "",
  ...
  "  Args: id (int)",
  "",
  "mem-agent_memory_observe — Phân loại tool execution qua observer.",
  "  Args: tool_name, tool_output, tool_input, session_id, cwd, tags",
  "",
  "mem-agent_memory_summarize — Tóm tắt session.",
  "  Args: session_id",
  "",
  "mem-agent_index_stats — Thống kê bộ nhớ.",
  "",
  "QUY TẮC:",
  "1. Đầu session → đọc memory gần đây. Nếu injected chưa đủ, gọi memory_list.",
  "2. Mỗi turn: user hỏi quá khứ/preference/task cũ → LUÔN memory_search.",
  "3. Trước khi edit file → memory_search với tên file/module/feature.",
  "4. Sau fix bug, quyết định, convention quan trọng → memory_add 1 lần.",
...
export function normalizeTag(tag: string): string {
  return tag.toLowerCase().replace(/[^a-z0-9._/-]+/g, "-").replace(/^-+|-+$/g, "");
}
...
export function safeSlice(v: unknown, max: number): string {
  if (typeof v === "string") return v.slice(0, max);
  if (v == null) return "";
  try { return JSON.stringify(v).slice(0, max); } catch { return ""; }
}

export function serializePayload(v: unknown, max: number): string {
  if (typeof v === "string") return v.slice(0, max);
  if (v == null) return "";
  try { return JSON.stringify(v, null, 2).slice(0, max); } catch { return safeSlice(v, max); }
}
...
export function summarizeModel(model: unknown): string {
...
export function extractFileParts(parts: unknown[]): string[] {
  return parts
    .filter((p) => p && typeof p === "object")
    .map((p) => p as Record<string, unknown>)
...
export function summarizeParts(parts: unknown[]): string[] {
  return parts
    .filter((p) => p && typeof p === "object")
    .map((p) => {
      const pp = p as Record<string, unknown>;
      return typeof pp.type === "string" ? pp.type : "";
    })
    .filter(Boolean);
}

export function makeTitle(prefix: string, text: string): string {
...
export function resolveMemAgentBin(): string {
  ...
  const releaseBin = path.resolve(REPO_ROOT, "target", "release", "mem-agent");
  if (existsSync(releaseBin)) return releaseBin;

  const debugBin = path.resolve(REPO_ROOT, "target", "debug", "mem-agent");
...
export function resolveDbPath(): string {
  return process.env.MEMAGENT_DB?.trim() || path.resolve(REPO_ROOT, "memories.db");
}

// ─── CLI Runner ────────────────────────────────────────

export async function runMemAgent(
  args: string[],
  timeoutMs = 8000,
): Promise<RunResult> {
...
export async function addMemory(
  ...
  const args = ["add", cleanTitle, cleanContent, "--tags", joinTags(tags)];
  ...
  if (!result.ok) {
    memLog("SAVE", COLOR.save_err, `${cleanTitle} — ${result.stderr || result.stdout}`);
  } else {
    memLog("SAVE", COLOR.save_ok, `${cleanTitle} [${joinTags(tags)}]`);
  }
}
...
export async function searchMemories(
  ...
  const result = await runMemAgent(
    ["search", cleanQuery, "--mode", "hybrid", "--scope", "auto", "-n", String(limit)],
    30000,
  );

  if (!result.ok || !result.stdout || result.stdout.includes("No results found")) {
    if (!result.ok) memLog("RECALL", COLOR.recall_err, cleanQuery);
    return "";
  }

  memLog("RECALL", COLOR.recall_ok, `${cleanQuery} → ${result.stdout.split("\n").length} lines`);
...
export async function listRecentMemories(
  ...
  const result = await runMemAgent(
    ["list", "--scope", "chat", "-n", String(limit)],
    8000,
  );
...
function trackBackground(label: string, task: () => Promise<void>): void {
  const job = task()
    .catch((error) =>
      memLog("ERR", COLOR.save_err,
        `${label}: ${error instanceof Error ? error.message : String(error)}`),
    )
    .finally(() => { backgroundTasks.delete(job); });
...
export function getCachedRecent(limit = RECENT_MEMORY_LIMIT): string {
  const entry = recentCache.get(limit);
  return isFresh(entry) ? entry!.value : "";
}

function recallCacheKey(query: string, limit: number): string {
  return `${limit}:${query}`;
}

export function getCachedRecall(query: string, limit: number): string {
  const entry = recallCache.get(recallCacheKey(query, limit));
  return isFresh(entry) ? entry!.value : "";
}
...
export function scheduleRecentRefresh(limit = RECENT_MEMORY_LIMIT): void {
  ...
  const job = (async () => {
    const value = await listRecentMemories(limit);
    recentCache.set(limit, { value, fetchedAt: Date.now() });
  })().finally(() => { pendingCacheJobs.delete(key); });

  pendingCacheJobs.set(key, job);
  trackBackground(key, () => job);
}
...
export function scheduleRecallRefresh(query: string, limit: number): void {
  ...
  const key = `recall:${recallCacheKey(cleanQuery, limit)}`;
  if (pendingCacheJobs.has(key) || isFresh(recallCache.get(recallCacheKey(cleanQuery, limit)))) {
    return;
  }

  const job = (async () => {
    const value = await searchMemories(cleanQuery, limit);
    recallCache.set(recallCacheKey(cleanQuery, limit), { value, fetchedAt: Date.now() });
  })().finally(() => { pendingCacheJobs.delete(key); });
...
export function enqueueMemorySave(
  title: string,
  content: string,
  tags: string[],
  opts?: {
  ...
  trackBackground(`save:${title}`, async () => {
    await addMemory(title, content, tags, opts);
...
export function createSessionState(projectPath: string): SessionState {
  return {
    ...
    seenToolCallIds:  new Map(),
    seenMessageIds:   new Map(),
    contextInjected:  new Set(),
    recentInjected:   new Set(),
    turnPrompts:      new Map(),
  };
}

export function sessionCleanup(sid: string, state: SessionState): void {
  state.trackedFiles.delete(sid);
  state.seenToolCallIds.delete(sid);
  state.seenMessageIds.delete(sid);
  state.contextInjected.delete(sid);
  state.recentInjected.delete(sid);
  state.turnPrompts.delete(sid);
}

export function fileSet(state: SessionState, sid: string): Set<string> {
  let s = state.trackedFiles.get(sid);
  if (!s) { s = new Set(); state.trackedFiles.set(sid, s); }
  return s;
}

export function toolCallSet(state: SessionState, sid: string): Set<string> {
  let s = state.seenToolCallIds.get(sid);
  if (!s) { s = new Set(); state.seenToolCallIds.set(sid, s); }
  return s;
}

export function messageIdSet(state: SessionState, sid: string): Set<string> {
  let s = state.seenMessageIds.get(sid);
  if (!s) { s = new Set(); state.seenMessageIds.set(sid, s); }
  return s;
}
